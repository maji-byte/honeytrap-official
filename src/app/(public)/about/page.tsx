    ...
    <>
      ...
      <section className="py-24 md:py-32 px-6 md:px-10 max-w-[1400px] mx-auto">
        <div className="max-w-3xl">
          ...
          <div className="space-y-8 font-body text-sm text-[var(--ht-text-muted)] leading-loose whitespace-pre-line">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {about.axes.map((axis, i) => (
              <motion.div
                ...
                className="p-6 bg-white border border-[var(--ht-border)] rounded-xl shadow-sm hover:shadow-md hover:border-[var(--ht-pink)]/30 transition-all"
              >
                <p className="text-3xl mb-4">{axis.icon}</p>
                <h3 className="font-heading text-lg font-bold text-[var(--ht-text)] mb-3">{axis.title}</h3>
                <p className="font-body text-xs text-[var(--ht-text-muted)] leading-relaxed">{axis.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 md:py-32 px-6 md:px-10 max-w-[1400px] mx-auto">
        <div className="max-w-3xl">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="w-24 h-24 bg-[var(--ht-bg-alt)] rounded-xl flex items-center justify-center shrink-0 overflow-hidden relative shadow-sm">
              {about.creator.image ? (
                <Image
                ...
                <span className="font-heading text-2xl text-[var(--ht-text)]/10">
                  {about.creator.name.charAt(0).toUpperCase()}
                </span>
              )}
            </div>
            <div className="space-y-4 font-body text-sm text-[var(--ht-text-muted)] leading-relaxed">
              <p>
                <strong className="text-[var(--ht-text)]">{about.creator.name}</strong><br />
                {about.creator.role}
              </p>
                ...
                <div className="flex flex-wrap gap-4 pt-2">
                  {about.creator.links.map((link, i) => (
                    <a
                      ...
                      className="text-xs font-heading tracking-wider text-[var(--ht-pink)] hover:text-[var(--ht-teal)] transition-colors"
                    >
                      {link.label} &rarr;
                    </a>
                  ))}
